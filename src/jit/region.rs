...
pub fn compile<'gc>(
    ...
    // SSA all the way to the allocator: selection does no parallel-copy
    // sequencing and the allocator builds intervals in one reverse pass rather
    // than iterating a live-set dataflow, resolving the edges itself as part of
    // the resolution it already had to do.
