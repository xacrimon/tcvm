...
fn lua_concat<'gc>(
    ...
    let t = check_table(ctx, stack.get(0), "concat")?;
    ...
    let mut k = i;
    while k <= j {
        let v = t.raw_get(Value::integer(ctx.mutation(), k));
...
fn lua_insert<'gc>(
    ...
    let t = check_table(ctx, stack.get(0), "insert")?;
    let n = t.raw_len() as i64;
    match stack.len() {
        2 => {
            t.raw_set(ctx, Value::integer(ctx.mutation(), n + 1), stack.get(1));
        }
        3 => {
            ...
            while k >= pos {
                let v = t.raw_get(Value::integer(ctx.mutation(), k));
                t.raw_set(ctx, Value::integer(ctx.mutation(), k + 1), v);
                k -= 1;
            }
            t.raw_set(ctx, Value::integer(ctx.mutation(), pos), stack.get(2));
        }
        _ => {
            return Err(Error::from_str(
                ctx,
...
fn lua_move<'gc>(
    ...
    let a1 = check_table(ctx, stack.get(0), "move")?;
    ...
    if e >= f {
        ...
        if t > e || t <= f || !same {
            for i in 0..n {
                let v = a1.raw_get(Value::integer(ctx.mutation(), f + i));
                a2.raw_set(ctx, Value::integer(ctx.mutation(), t + i), v);
            }
        } else {
            let mut i = n - 1;
            loop {
...
fn lua_remove<'gc>(
    ...
    let t = check_table(ctx, stack.get(0), "remove")?;
    let n = t.raw_len() as i64;
    ...
    let mut k = pos;
    while k < n {
        let v = t.raw_get(Value::integer(ctx.mutation(), k + 1));
        t.raw_set(ctx, Value::integer(ctx.mutation(), k), v);
        k += 1;
    }
    t.raw_set(
        ctx,
        Value::integer(ctx.mutation(), pos.max(n)),
        Value::nil(),
    );
    stack.ret1(result);
    Ok(CallbackAction::Return)
...
fn lua_sort<'gc>(
    ...
    let t = check_table(ctx, stack.get(0), "sort")?;
    let n = t.raw_len();
    if n <= 1 {
...
fn lua_unpack<'gc>(
    ...
    let t = check_table(ctx, stack.get(0), "unpack")?;
    ...
    let mut k = i;
    while k < j {
        out.push(t.raw_get(Value::integer(ctx.mutation(), k)));
        k += 1;
    }
    out.push(t.raw_get(Value::integer(ctx.mutation(), j)));
    stack.replace(&out);
    Ok(CallbackAction::Return)
}
