        ...
        match e {
            RuntimeError::Lua(stashed) => {
                let msg = lua.enter(|ctx| {
                    ...
                    // Like lua.c's msghandler: strings and numbers print
                    // as-is, anything else by type.
                    let v = err.value();
                    if v.get_string().is_some()
                        || v.get_integer().is_some()
                        || v.get_float().is_some()
                    {
                        let s = tcvm::basic_tostring(ctx, v);
